...
| **unirtos 工具链** | 提供 `unirtos` 命令用于编译 | 1.0.6+ |
...
| `-f`, `--force` | 关闭 | 强制更新 `unirtos-demos-manifests` 后再选 demo。仅可与 `-r` 同时使用。 |
...
| 参数 | 说明 |
| --- | --- |
| `-f`, `--force` | 强制更新 `unirtos-demos-manifests`（忽略 1 小时更新间隔）。 |
